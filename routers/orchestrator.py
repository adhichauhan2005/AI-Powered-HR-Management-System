...
def route_question_node(
    ...
    - SQL Agent    (data/numbers from the database)
    - RAG Agent    (HR policies, rules, guidelines from documents)
    - Analytics Agent (risk detection, anomaly detection, scorecards)

    Routing priority:
    1. Analytics  — risk, anomaly, scorecard, pattern detection
    2. RAG        — policy, rule, guideline, eligibility (narrow)
    3. SQL        — any data question (broad, catches most queries)
    4. Default    — SQL (safer default for data-heavy HR system)
    """
    ...
    # -----------------------------------------------------
    # Checked first because words like "attendance" and
    # "payroll" may also appear in SQL questions.
    analytics_keywords = [
        "risk",
        "risky",
        "anomaly",
    ...
    # -----------------------------------------------------
    # STEP 2: Policy/privacy/access/rules questions (RAG)
    # -----------------------------------------------------
    # Only match questions that are truly about HR policies,
    # rules, guidelines, eligibility, or access rights.
    # Keep these NARROW to avoid stealing data questions.
    rag_keywords = [
        # Access & privacy
        "who can access",
        "access payroll",
        ...
        "procedures",
        "protocol",
        # Eligibility & approval workflows
        "eligible",
        "eligibility",
        "qualify",
        "qualification",
    ...
    if any(keyword in question_lower for keyword in rag_keywords):
        return {
            "selected_agent": "rag",
            "route_reason": (
                "The question is about HR policy, rules, guidelines, "
                "access rights, or eligibility, so it should be "
                "answered from policy documents."
            ),
            "answer_type": "policy_document_answer",
        }

    # -----------------------------------------------------
    # STEP 3: Structured SQL data questions (BROAD)
    # -----------------------------------------------------
    # This catches ALL data-retrieval questions. Any question
    # asking for employee data, names, counts, lists, salaries,
    # departments, attendance, training, etc. goes to SQL.
    sql_keywords = [
        # --- Employee & people terms ---
        "employee",
        "employees",
        "staff",
        "worker",
        ...
        # --- Action verbs for data retrieval ---
        "show",
        "list",
        "display",
        "give",
    ...
    if any(keyword in question_lower for keyword in sql_keywords):
        return {
            ...
            "route_reason": (
                ...
                "question that should be answered from the "
                "HR database."
            ),
    ...
    # Default route: SQL
    # ---------------------------------------------------------
    # In an HR management system, most questions are about
    # employee data. Defaulting to SQL is safer than RAG
    # for unrecognized questions.
    return {
        "selected_agent": "sql",
        "route_reason": (
            "The question was not clearly matched to a specific "
            "agent, so it was routed to the SQL Agent as the "
...
def sql_agent_node(
    ...
    # Handle case where detect_intent returns None (no matching template)
    if intent is None or intent not in QUERY_TEMPLATES:
        return {
            "selected_agent": "sql",
            "route_reason": (
                "The orchestrator routed the question to SQL, "
                "but no matching SQL intent was found in the training data."
            ),
            "answer_type": "sql_agent_no_match",
            "answer": (
                "I could not confidently match your question to a "
                "trained SQL intent. Please rephrase or ask a specific "
                "HR database question."
            ),
            "results": [],
            "row_count": 0,
        }
