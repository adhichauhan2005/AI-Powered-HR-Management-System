...
def calculate_example_match_score(question: str, example: str) -> float:
    ...
    Uses bidirectional word overlap + keyword boosting.
    """
    ...
    # Remove common stop words for better matching accuracy
    stop_words = {
        "a", "an", "the", "is", "are", "was", "were", "be",
        "been", "being", "do", "does", "did", "have", "has",
        "had", "will", "would", "shall", "should", "may",
        "might", "can", "could", "of", "in", "to", "for",
        "with", "on", "at", "by", "from", "it", "its",
        "this", "that", "these", "those", "i", "me", "my",
        "we", "our", "you", "your", "and", "or", "but",
        "if", "then", "so", "than", "too", "very", "just",
        "about", "up", "out", "there", "here", "all", "each",
        "any", "some", "no", "not", "only", "also", "please",
    }

    question_content = question_words - stop_words
    example_content = example_words - stop_words

    if not question_content or not example_content:
        # Fall back to full word sets if filtering removes everything
        question_content = question_words
        example_content = example_words

    if not question_content or not example_content:
        return 0.0

    common_words = question_content.intersection(example_content)

    # Bidirectional overlap: take the higher of both directions
    # Direction 1: how many example words are in the question
    score_example_to_question = len(common_words) / len(example_content)
    # Direction 2: how many question words are in the example
    score_question_to_example = len(common_words) / len(question_content)

    # Use the higher score for more forgiving matching
    score = max(score_example_to_question, score_question_to_example)

    # Keyword boost: if both share important HR domain words, boost score
    important_keywords = {
        "employee", "employees", "department", "departments",
        "salary", "salaries", "payroll", "attendance",
        "training", "performance", "rating", "leave",
        "overtime", "bonus", "certification", "skill",
        "skills", "manager", "budget", "absent", "late",
        "promotion", "names", "name", "list", "count",
        "total", "average", "highest", "lowest", "top",
        "pending", "active", "inactive",
    }

    shared_important = common_words.intersection(important_keywords)
    if shared_important:
